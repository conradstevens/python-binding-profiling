...
def test_intree_extensions_package_dir(monkeypatch, tmpdir):
    ...
    monkeypatch.chdir(tmpdir)
    root = tmpdir / "pure_python_package"
    root.ensure_dir()
    subdir = root / "dir"
    subdir.ensure_dir()
    src = subdir / "ext.cpp"
    src.ensure()
    (ext,) = intree_extensions([src.relto(tmpdir)], package_dir={"": "pure_python_package"})
    assert ext.name == "dir.ext"
    (ext,) = intree_extensions([src.relto(tmpdir)], package_dir={"foo": "pure_python_package"})
